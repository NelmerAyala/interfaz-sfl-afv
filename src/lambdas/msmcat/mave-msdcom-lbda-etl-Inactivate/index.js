...
const { deleteRutaDetalle } = require("./queries/query");
...
exports.handler = async (event) => {
  ...
  // No es necesario un Loop, ya que se recibe una entidad de Marcacatalogo.
  ...
  for (const item of Object.keys(response.data)) {
    if (item === "marcaCatalogo")
      await deleteMarcacatalogoByID({ action, schema, data: response.data[item] });
  }
